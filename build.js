...
return Promise.resolve()
  ...
  .then(() => {
    // Base configuration.
    const es5Entry = path.join(es5OutputFolder, `${fileName}.js`);
    const es2015Entry = path.join(es2015OutputFolder, `${fileName}.js`);
    ...
    const umdConfig = Object.assign({}, rollupBaseConfig, {
      entry: es5Entry,
      dest: path.join(distFolder, `bundles`, `${fileName}.umd.js`),
      format: 'umd',
    });
    ...
    const minifiedUmdConfig = Object.assign({}, rollupBaseConfig, {
      entry: es5Entry,
      dest: path.join(distFolder, `bundles`, `${fileName}.umd.min.js`),
      format: 'umd',
    ...
    const fesm5config = Object.assign({}, rollupBaseConfig, {
      entry: es5Entry,
      dest: path.join(distFolder, `${fileName}.es5.js`),
      format: 'es'
    });
    ...
    const fesm2015config = Object.assign({}, rollupBaseConfig, {
      entry: es2015Entry,
      dest: path.join(distFolder, `${fileName}.js`),
      format: 'es'
    });
